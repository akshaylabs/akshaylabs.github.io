...
async def send_media_with_progress(
    ...
    sent_msg = None
    for attempt in range(1, 4):   # up to 3 upload attempts
        try:
            ...
            # FLOOD_PREMIUM_WAIT (420) on upload — sleep exact duration + buffer
            wait_s = int(getattr(e, "value", 0) or 15)
            LOGGER(__name__).warning(f"FloodPremiumWait {wait_s}s on upload (attempt {attempt}/3)")
            if attempt < 3:
                await asyncio.sleep(wait_s + 2)
                continue
            raise
        except FloodWait as e:
            # FLOOD_WAIT (420) on upload — sleep exact duration Telegram demands
            wait_s = int(getattr(e, "value", 0) or 30)
            LOGGER(__name__).warning(f"FloodWait {wait_s}s on upload (attempt {attempt}/3)")
            if attempt < 3:
                await asyncio.sleep(wait_s + 2)
                continue
            raise
        except Exception as e:
            # Any other upload error — short backoff then retry once more
            LOGGER(__name__).error(f"Upload error attempt {attempt}/3: {e}")
            if attempt < 3:
    ...
    async def _upload_slot(index: int):
        ...
        if result.media_type == "group" and result.media_path:
            ...
            if group_sent_count > 0:
                sent += 1
                if index in state_mgr.state_map:
                    st = TaskStatus.PARTIAL if group_failed_count > 0 else TaskStatus.COMPLETED
                    state_mgr.state_map[index].status = st
            else:
                failed += 1
                if index in state_mgr.state_map:
                    state_mgr.state_map[index].status = TaskStatus.ABANDONED
        ...
        if result.media_path:
            ...
            ph_id = sonic_ph_map.get(index)
            if index in state_mgr.state_map:
                state_mgr.state_map[index].status = TaskStatus.UPLOADING
            ...
            if replaced_in_place:
                ...
                if index in state_mgr.state_map:
                    state_mgr.state_map[index].status = (
                        TaskStatus.PARTIAL if result.is_partial else TaskStatus.COMPLETED)
                if src_msg and result.msg:
                    # We can't get the msg_id back from edit_message_media easily,
                    # so store the placeholder id as the sent position
                    src_to_dst_map[src_msg.id] = ph_id
                    if index in state_mgr.state_map:
                        state_mgr.state_map[index].sent_msg_id = ph_id
            else:
        ...
        if skip_event.is_set():
            ...
            for idx in range(n):
                if idx not in results and idx not in pending_uploads:
                    if idx in state_mgr.state_map:
                        state_mgr.state_map[idx].status = TaskStatus.SKIPPED
                    skipped += 1
                    completed_count += 1
...
async def _strict_sonic_sender(
    ...
    async def _send_real(index: int):
        ...
        if result.media_type == "group" and result.media_path:
            ...
            if group_sent_count > 0:
                sent += 1
                if index in state_mgr.state_map:
                    st = (TaskStatus.PARTIAL if group_failed_count > 0
        ...
        if result.media_path:
            ...
            if replaced_in_place:
                ...
                if index in state_mgr.state_map:
                    state_mgr.state_map[index].status = (
                        TaskStatus.PARTIAL if result.is_partial
                        else TaskStatus.COMPLETED)
                if src_msg and result.msg:
                    src_to_dst_map[src_msg.id] = ph_id
                    if index in state_mgr.state_map:
                ...
                # edit_message_media failed — fall back to new message
                try:
                    sent_msg = await forward_message(
                        src_msg=src_msg,
                        chat_id=chat_id,
                    ...
                    LOGGER(__name__).info(
                        f"[StrictSonic] ✅ Slot {index} sent (fallback) {fmt_size(fsize)}")
                    sent += 1 if not result.is_partial else 0
                    failed += 1 if result.is_partial else 0
                    if index in state_mgr.state_map:
                        state_mgr.state_map[index].status = (
                    ...
                    if sent_msg and src_msg:
                        src_to_dst_map[src_msg.id] = sent_msg.id
                    # Update gap placeholder with jump link
                    if ph_id and sent_msg:
                        try:
                            link = (f"https://t.me/c/"
                                    f"{str(chat_id).lstrip('-100')}/{sent_msg.id}")
                            ...
                            LOGGER(__name__).debug(
                                f"[StrictSonic] Jump-link update failed: {e}")
                except Exception as e:
                    failed += 1
                    cleanup_download(result.media_path)
                    LOGGER(__name__).error(
                        f"[StrictSonic] Upload failed slot {index}: {e}")
                    if index in state_mgr.state_map:
                        state_mgr.state_map[index].status = TaskStatus.FAILED
                        state_mgr.state_map[index].error  = str(e)
            return
    ...
    # Phase B: for each slot in pending_replace, wait for ANY of the
    #   remaining ready_events (not strictly in order) so that a slow
    #   slot can't block faster ones from being replaced. Chat order is
    #   already preserved because every slot has a placeholder in chat.
    async def chat_writer():
        ...
        while write_pointer < n:
            if kill_event.is_set():
                while write_pointer < n:
                    if write_pointer in state_mgr.state_map:
                        state_mgr.state_map[write_pointer].status = TaskStatus.SKIPPED
                    skipped += 1
                    write_pointer += 1
                return
        ...
        # FIX: Process whichever slot becomes ready FIRST, not in fixed order.
        # Because every slot already has a placeholder in chat, we don't need
        # strict order here — chat sequence is already preserved visually.
        # Waiting in fixed order caused all later slots to stay stuck in
        # SENDING (queued) while a single slow earlier slot blocked them.
        remaining = list(pending_replace)
        while remaining and not kill_event.is_set():
            # Build a mapping of wait-task → slot index for all remaining slots
            wait_map: Dict[asyncio.Task, int] = {}
            for idx in remaining:
                if not ready_events[idx].is_set():
                    t = asyncio.ensure_future(ready_events[idx].wait())
                else:
                    # Already ready (download finished before Phase B started)
            ...
            # Cancel the remaining wait tasks so they don't accumulate
            for t in wait_map:
            ...
            for done_task in done_tasks:
                ...
                if kill_event.is_set():
                    break
                LOGGER(__name__).info(
                    f"[StrictSonic] writer: replacing placeholder for slot {idx}")
                await _send_real(idx)
                await state_mgr.move_status_to_bottom()
                break  # restart outer while so wait_map is rebuilt for remaining
